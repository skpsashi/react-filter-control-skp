...
const Condition = ({ fields, removeClick, onConditionValueChanged, value }) => {
  const getActiveField = (conditionValue) => {
    const index = fields.findIndex(
      (field) => field.name === conditionValue.field
    );
    const activeField = fields[index];
    const activeOperatorIndex = activeField.operators.findIndex(
      (operator) => operator.name === conditionValue.operator
    );
    return {
      index,
      activeOperatorIndex,
  ...
  const handleValueChanged = (e) =>
    onConditionValueChanged(updateCondition("value", e.target.value));
  const handleFieldChanged = (newValue) =>
    onConditionValueChanged(updateCondition("field", newValue));
  const handleOperatorChanged = (newValue) =>
    onConditionValueChanged(updateCondition("operator", newValue));
